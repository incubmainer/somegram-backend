    ...
    >
{
  constructor(
    ...
    this.logger.setContext(GetFollowersUseCase.name);
  }
  async execute(
    command: GetFollowersQuery,
  ): Promise<
    AppNotificationResultType<Pagination<FollowingProfileOutputDtoModel[]>>
  > {
    const { userId, currentUserId, queryString, cursorUserId } = command;
    this.logger.debug(
      `Execute: get followers for userId ${userId}`,
      this.execute.name,
    );

    try {
      const { users, count } = await this.usersQueryRepository.getFollowers(
        userId,
        queryString,
        cursorUserId,
      );
