...
describe('Delete user avatar', () => {
  ...
  beforeAll(async () => {
    ...
    commandBus.register([UploadAvatarUseCase, DeleteAvatarUseCase]);
  });
  ...
  it('should upload then should delete user avatar, by user id', async () => {
    ...
    const deleteAvatar: AppNotificationResultType<null> =
      await commandBus.execute(new DeleteAvatarCommand(userId));
    expect(deleteAvatar.appResult).toBe(AppNotificationResultEnum.Success);
    expect(deleteAvatar.errorField).toBeNull();
    expect(deleteAvatar.data).toBeNull();
  });

  it('should not delete avatar, user not found', async () => {
  ...
  it('should not delete avatar, some internal server error', async () => {
    const spy = jest
      .spyOn(photoServiceAdapterMock, 'deleteAvatar')
