...
export class PostsQueryRepository {
  ...
  public async getPostsByUserIds(
    ...
    this.logger.debug(
      `Execute: get posts by userIds: ${userIds.join(', ')}`,
    ...
    let whereClause = Prisma.sql`p."userId" = ANY(ARRAY[${Prisma.join(userIds)}]) and ubi."userId" IS NULL`;
    ...
    if (sanitizationQuery.sortBy === 'createdAt' && endCursorCreatedAt) {
      ...
      const operator = sanitizationQuery.sortDirection === 'asc' ? '>' : '<';
      whereClause = Prisma.sql`${whereClause} and p."updatedAt" ${Prisma.raw(operator)} ${Prisma.raw(`'${endCursorUpdatedAt.toISOString()}'`)}`;
    }
