...
pipeline {
    ...
    environment {
        ENV_TYPE = "production"
        PORT = 3443
        NAMESPACE = "somegram-online"
        REGISTRY_HOSTNAME = "incubmainer"
        PROJECT = "somegram-backend"
        SERVICE="gateway"
        REGISTRY = "registry.hub.docker.com"
        DEPLOYMENT_NAME = "somegram-backend-deployment"
        ...
        stage('Push docker image') {
             steps {
                 echo "Push image started..."
                     script {
                        docker.withRegistry("https://${env.REGISTRY}", 'somegram-online') {
